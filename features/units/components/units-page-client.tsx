...
import { useDeleteUnit, useUnits } from "../hooks/user-units";
...
export default function UnitsPageClient() {
  ...
  const unitsQuery = useUnits({ search, page, limit, isActive });
  const deleteUnit = useDeleteUnit();

  const units = unitsQuery.data?.data.units || [];
  const pagination = unitsQuery.data?.data.pagination;

  const selectedUnits = units.filter((unit) =>
    selectedRowKeys.includes(unit.id),
  );
  ...
  function openCreatePage() {
    router.push("/master_data/units/new");
  }

  function openDetailPage(unit: Unit) {
    router.push(`/master_data/units/${unit.id}`);
  }

  function openEditPage(unit: Unit) {
    router.push(`/master_data/units/${unit.id}/edit`);
  }
  ...
  const bulkItems: MenuProps["items"] = [
    {
      ...
      onClick: () => {
        if (selectedUnits.length !== 1) {
          message.warning("Pilih 1 satuan saja untuk edit.");
          return;
        }

        openEditPage(selectedUnits[0]);
      },
    },
    {
      key: "delete",
      ...
      danger: true,
      onClick: () => {
        selectedRowKeys.forEach((id) => deleteUnit.mutate(String(id)));
        setSelectedRowKeys([]);
      },
    },
  ];

  const allColumns: ColumnsType<Unit> = [
    {
      title: "Name",
      dataIndex: "name",
      key: "name",
    ...
    <div className="flex flex-col gap-5">
      ...
      >
        <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
          <FilterField label="Name / Symbol" span="">
            <FilterInput
              value={searchInput}
